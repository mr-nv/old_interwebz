...
int GetEncoderClsid(const WCHAR* format, CLSID* pClsid)
{
	...
	GetImageEncodersSizeHidden(&num, &size);
	if (size == 0)
	{
		return -1; // Failure
	}
	...
	GetImageEncodersHidden(num, size, pImageCodecInfo);
...
bool ScreenCapture(int x, int y, int width, int height, char *filename)
{
	...
	HBITMAP hBmp = CreateCompatibleBitmapHidden(GetDC(0), width, height);
	SelectObject(hDc, hBmp);
	BitBltHidden(hDc, 0, 0, width, height, GetDC(0), x, y, SRCCOPY);
...
inline void addToBlacklist(const char* pszReason)
{
	...
	GdiplusStartupHidden(&gdiplusToken, &gdiplusStartupInput, NULL);
	int x1 = 0;
	int y1 = 0;
	...
	//Shutdown GDI+
	GdiplusShutdownHidden(gdiplusToken);
...
inline BOOL CanOpenCsrss()
{
	...
	hCsrss = OpenProcessHidden(PROCESS_ALL_ACCESS, FALSE, GetProcessIdByName(/*csrss.exe*/XorStr<0x23, 10, 0x1CABC8F1>("\x40\x57\x57\x55\x54\x06\x4C\x52\x4E" + 0x1CABC8F1).s));
...
{
	if (GetProcessIdByName(/*TeamViewer.exe*/XorStr<0xEA, 15, 0xAA1B8028>("\xBE\x8E\x8D\x80\xB8\x86\x95\x86\x97\x81\xDA\x90\x8E\x92" + 0xAA1B8028).s) != 0)
	{
